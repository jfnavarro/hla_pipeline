...
#     position = int(re.findall(r'\d+', protein_mut)[0]) - 1
#     # nonsynonymous point mutations
#     if 'missense_variant' in exonic_func:
#         # extract the AA change info
#         ref_AA = protein_letters_3to1[re.search('\.(.+?)\d+', protein_mut).group(1)]
#         var_AA = protein_letters_3to1[protein_mut[-3::]]
#         # gather AA seq for transcript
#         if protein_seq == 'None':
#             errors.append('AA seq not present for this transcript')
#         else:
...
#                 len_del = len(ref) - len(alt)
#                 mut_cDNA_seq = cDNA_seq[:cDNA_pos] + cDNA_seq[cDNA_pos + len_del:]
#             elif 'dup' in cDNA_mut:
#                 # dup = len(alt) - len(ref)
#                 # mut_cDNA_seq = cDNA_seq[:cDNA_pos] + cDNA_seq[cDNA_pos:cDNA_pos+dup] * 2 + cDNA_seq[cDNA_pos+dup:]
#                 if len(cDNA_mut.split('_')) > 1:
#                     dup_pos = int(re.search('_(.*)dup', cDNA_mut).group(1))
#                     mut_cDNA_seq = cDNA_seq[:cDNA_pos] + cDNA_seq[cDNA_pos:dup_pos] * 2 + cDNA_seq[dup_pos:]
#                 else:
#                     dup_pos = int(re.search('\.(.*)dup', cDNA_mut).group(1))
#                     mut_cDNA_seq = cDNA_seq[:cDNA_pos] + cDNA_seq[cDNA_pos:dup_pos] * 2 + cDNA_seq[dup_pos:]
#             elif 'ins' in cDNA_mut:
#                 ins = cDNA_mut[int(cDNA_mut.find('ins')) + 3:]
#                 mut_cDNA_seq = cDNA_seq[:cDNA_pos] + ins + cDNA_seq[cDNA_pos:]
#             elif 'delins' in cDNA_mut:
#                 subs = cDNA_mut[int(cDNA_mut.find('delins')) + 6:]
#                 len_subs = len(subs)
#                 mut_cDNA_left = cDNA_seq[:cDNA_pos]
#                 mut_cDNA_right = cDNA_seq[cDNA_pos + len_subs:]
